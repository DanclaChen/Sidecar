...
####### 2. PDE error       
    ...
    def train(self):
        ...
        torch.save(self.model.state_dict(), f"./trained_model/1DBurgers-PINNs_withR-bs{self.batch_size}-epoches{self.epoches}-h_train{self.h_train}-tau_train{self.tau_train}-gamma{self.gamma}-width{self.width}-hidden{self.num_hidden}-widthR{self.width_R}-hiddenR{self.num_hidden_R}-LBFGS{self.Adam_proportion}-{self.j}.pth")
