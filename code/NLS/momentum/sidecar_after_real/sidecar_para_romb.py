...
class structured_PINNs():
    def __init__(self, params, j, device):
        ...
        self.model.load_state_dict(torch.load(f"/Sidecar/code/NLS/mass/PINNs_withR/trained_model/NLS-romb-bs{self.batch_size}-epoches100000-width{self.width}-depth{self.num_hidden}-widthR{self.width_R}-depthR{self.num_hidden_R}-n_bic{self.n_bic_points}-n_inner{self.n_inner_points}-gamma0.99999-{self.j}.pth", map_location= f"cuda:{self.device}"))
        self.x_l = params.x_l
        self.x_r = params.x_r
        self.t_0 = params.t_0
        self.t_T = params.t_T
    ...
    def loss_fn(self, X, x_inner, t_inner, x, t, labels, epoch):
        ...
        du_dx_re = du_dX_re[:, 0]
        du_dx_im = du_dX_im[:, 0]
        du_dxx_re = self.gradient(du_dx_re, X)[:, 0]
        du_dxx_im = self.gradient(du_dx_im, X)[:, 0]
        ...
        dv_dx_re = self.gradient(v_pred_re, X)[:, 0]
        dv_dx_im = self.gradient(v_pred_im, X)[:, 0]
        integral_temp = v_pred_im * dv_dx_re - v_pred_re *dv_dx_im
        ...
        loss_structure_vector = (mass_pred - torch.tensor(self.NLS.C_2).repeat(len(t_inner)).to(self.device))**2
        ...
        momentum_temp = u_pred_im * du_dx_re - u_pred_re * du_dx_im
        ...
        loss_energy_vector = np.abs(temp - self.NLS.C_2)
    ...
    def train(self):
        ...
        torch.save(self.model.state_dict(), f"./trained_model/NLS-after-romb-lr{self.learning_rate}-bs{self.batch_size}-epoches{self.epoches}-width{self.width}-depth{self.num_hidden}-widthR{self.width_R}-depthR{self.num_hidden_R}-n_bic{self.n_bic_points}-n_inner{self.n_inner_points}-coeR{self.coe_structure}-gamma{self.gamma}-{self.j}.pth")
        writer.close()
    def evaluation(self):
