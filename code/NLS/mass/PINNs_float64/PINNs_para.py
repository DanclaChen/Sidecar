...
class bic_Dataset(Dataset):
    def __init__(self, n_points, x_l, x_r, t_0, t_T):
        ...
        x_initial = torch.linspace(x_l, x_r, n_points * 4).unsqueeze(1)
        t_initial = torch.zeros(n_points * 4, 1)
        ...
        self.labels = torch.cat([torch.ones(n_points * 4), torch.full((n_points,), 2)], dim=0).long()
...
class structured_PINNs():
    def __init__(self, params, j, device):
        ...
        self.t_0 = params.t_0
        self.t_T = params.t_T
        self.x_train = torch.linspace(self.x_l, self.x_r, self.n_inner_points * 4)
    ...
    def loss_fn(self, X, x, t, labels, epoch):
        if len(x[labels == 2]) == 0:
            ...
            t_bc = t[labels == 2]
            u_pred_bc1 = self.model(torch.cat([x_bc1, t_bc], dim=1))
            u_pred_bc2 = self.model(torch.cat([x_bc2, t_bc], dim=1))
            u_pred_bc1_re, u_pred_bc1_im = torch.unbind(u_pred_bc1, dim=1)
            u_pred_bc2_re, u_pred_bc2_im = torch.unbind(u_pred_bc2, dim=1)
            du_dX_bc1_re = self.gradient(u_pred_bc1_re, x_bc1)
            du_dx_bc1_re = du_dX_bc1_re[:, 0]
            du_dX_bc1_im = self.gradient(u_pred_bc1_im, x_bc1)
            du_dx_bc1_im = du_dX_bc1_im[:, 0]
            du_dX_bc2_re = self.gradient(u_pred_bc2_re, x_bc2)
            du_dx_bc2_re = du_dX_bc2_re[:, 0]
            du_dX_bc2_im = self.gradient(u_pred_bc2_im, x_bc2)
            du_dx_bc2_im = du_dX_bc2_im[:, 0]
    ...
    def closure(self):
        ...
        loss_vector = self.loss_fn(self.X_train, self.x_LBFGS, self.t_LBFGS, self.labels_LBFGS, self.current_epoch)
        loss_vector[0].backward()
        return loss_vector[0]

    def train(self):
        writer = SummaryWriter(comment=f"NLS-Original-bs{self.batch_size}-epoches{self.epoches}-width{self.width}-n_bic{self.n_bic_points}-n_inner{self.n_inner_points}-gamma{self.gamma}-{self.j}")
        self.model.train()
        for epoch in range(self.epoches):
            if epoch < (self.Adam_proportion * self.epoches):
                for x, t, labels in self.bic_loader:
                    ...
                    loss_vector = self.loss_fn(self.X_train, x, t, labels, epoch)
                    loss_vector[0].backward()
                ...
                for x, t, labels in self.bic_loader_LBFGS:
                    ...
                    loss_vector = self.loss_fn(self.X_train, self.x_LBFGS, self.t_LBFGS, self.labels_LBFGS, epoch)
        ...
        torch.save(self.model.state_dict(), f"./trained_model/NLS-Original-bs{self.batch_size}-epoches{self.epoches}-width{self.width}-n_bic{self.n_bic_points}-n_inner{self.n_inner_points}-gamma{self.gamma}-{self.j}.pth")
        writer.close()

    def evaluation(self):
        x_test = torch.linspace(self.x_l, self.x_r, self.n_points_test * 2)
        ...
        t_ic_test = torch.zeros(self.n_points_test * 2, 1)
        ...
        labels = torch.cat([torch.ones(self.n_points_test * 2), torch.full((self.n_points_test,), 2)], dim=0).long().to(self.device)
        self.model.eval()
        loss_vector = self.loss_fn(X_test, x, t, labels, self.epoches)
