...
class structured_PINNs():
    ...
    def loss_fn(self, X, x_inner, t_inner, x, t, labels, epoch):
        ...
        temp = integrate.romb(u_square_matrix, dx = h.cpu().numpy(), axis = 0)
        loss_energy = np.max(np.abs(temp - self.NLS.C_1))
    ...
    def train(self):
        ...
        torch.save(self.model.state_dict(), f"./trained_model/NLS-romb-bs{self.batch_size}-epoches{self.epoches}-width{self.width}-depth{self.num_hidden}-widthR{self.width_R}-depthR{self.num_hidden_R}-n_bic{self.n_bic_points}-n_inner{self.n_inner_points}-gamma{self.gamma}-{self.j}.pth")
        writer.close()
    def evaluation(self):
